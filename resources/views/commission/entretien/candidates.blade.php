...
$commission = App\Models\Commission::where('email',Auth::user()->email)->select('interview_members')->first();
@endphp
...
<div class="container">
    ...
    <div class="inline-block text-end px-4 mx-4">
            <a target="_blank" class="btn btn-info @if (!$commission->interview_members) disabled @endif" href="{{ route('pv.entretien') }}">
                <i class="fas fa-print"></i> Générer PV
            </a>
    </div>
    <h5 class="mx-4 mt-2 text-muted"><strong>Les membres de cette commission</strong></h5>
    @if (!$commission->interview_members)
    ...
    @else
    <div class="mx-5">{!! $commission->interview_members !!}</div>
    @endif
                    ...
                    <tr>
                        ...
                        <td>
                            @if (App\Models\Note::where('dossier_id', $item->id))
                                    ...
                                    @endphp
                                    @if (is_float($notes->entretien_1))
                                    {{ $notes->entretien_1+$notes->entretien_2+$notes->entretien_3+$notes->entretien_4 }}
                                    @else
                                    <button class="@if (!$commission->interview_members) disabled @endif btn btn-success text-white fw-bold" data-bs-target="#Entretien{{ $item->id }}" data-bs-toggle="modal">Noter</button>
                                    @endif
                            @endif
                            </td>
                    </tr>
